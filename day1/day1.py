...
current_elf_inventory_total = 0

with open("./input", "r") as elf_file:
    for line in elf_file:
        if line in ["\n", "\r\n"]:
            max_elf_inventory_total = max(max_elf_inventory_total, current_elf_inventory_total)
            current_elf_inventory_total = 0
        else:
            current_elf_inventory_total += int(line)
...
with open("./input", "r") as elf_file:
    for line in elf_file:
        if line in ["\n", "\r\n"]:
            ...
            top_three_elfs.push(current_elf_inventory_total, current_elf_inventory_total)
